...
2b. **SEMANTIC OPTIMIZATION** — after the offer is chosen, before any
   ...
   b. For each segment, list its SEMANTIC WORDS — the exact words that
      ...
      shop", "customers walking in", "videos"). Name real things the
      owner knows — "Instagram", "Facebook", "Google" — never vague
      stand-ins like "page", "profile", "online presence". And never
      narrow the audience (city, business types) beyond the targeting the
      founder set — if he said "businesses in Thailand", the hook says
      that, not "cafes in Bangkok".
...
1. Voiceover total length: **under 20 seconds** (~45 spoken words max).
2. **Short sentences. Simple words.** Nothing longer than ~8 words per
   sentence. No hard vocabulary ("consistently", "presence", "optimize" are
...
4b2. FREE vs PAID clarity: when an offer mixes free and paid parts, the
   ...
   videos. Never hide the paid step to make the ad sound sweeter, and do
   NOT use labels like "trial" or "demo" either — the founder wants the
   split stated in plain words, not marketing labels.
...
2. **Hook options, derived FROM the chosen offer** (hooks are written
   ...
   element EARLY — inside the first 3 seconds — but never as a naked
   first word with no context ("FREE!" alone reads as spam and answers
   nothing). Hook order for offer-led: WHO it's for → WHAT it is → the
   irresistible element ("Business in Thailand? 3 videos of your shop —
   free — to bring you customers."). The element lands on something the
